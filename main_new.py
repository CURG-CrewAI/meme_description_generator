...
class MemeAgentCrew:
    def __init__(self):
        ...
        self.extractor_agent = Agent(
            ...
            다음 AI Agent들의 기사글 요약 및 번역, 풍자글 작성에 필요한 정보를 제공합니다.
        ...
        self.translator_agent = Agent(
            role='기사 요약 및 영어 번역 전문가',
            goal='한국 기사를 적절한 길이로 요약하고 영어로 번역',
            backstory=("""
            당신은 추출한 한국 뉴스들을 글로벌 독자들이 이해하기 쉽게 
            취합하고 요약하고 번역하는 전문가입니다.
            복잡한 한국 이슈를 외국인도 쉽게 이해할 수 있도록
            맥락과 배경을 포함해 명확하게 번역합니다."""
            ),
            verbose=True,                             
    ...
    def create_extraction_task(self):
        return Task(
            ...
            6. 접근 불가하거나 추출 실패한 URL은 건너뛰고 계속 진행
            7. 성공한 기사들만으로 결과 작성
            8. 최소 1개 이상의 기사가 성공적으로 추출되면 작업 완료로 처리

            에러 처리:
            - URL 접근 실패, 스크래핑 차단 등은 해당 기사만 제외
            - 실패한 기사는 로그에 기록하되 전체 작업을 중단하지 않음
            - 추출 가능한 기사들로만 extracted_content 배열 구성
    ...
    def create_translation_task(self):
        return Task(
            description="""
            이전 단계에서 추출된 JSON 형식의 기사 본문들을 취합하여 요약하고 영어로 번역하세요.
            
            처리 과정:
            1. extracted_content 배열의 모든 기사 내용들을 종합 분석
            2. 여러 기사의 공통 주제와 핵심 내용 파악
            3. 이미지 생성에 적합한 핵심 키워드 2~3개 추출(핵심 인물명, 장소, 핵심 개념, 핵심 사물 등)
            - 예시: "Son Heung-min, LAFC Stadium, Soccer Ball" 또는 "Lee Jae-myung, 250,000 won"
            - 키워드는 구체적이고 검색 가능한 명사 위주로 선택
            - 추상적 개념보다는 시각적으로 표현 가능한 요소 우선 
            4. 전체 이슈를 200-300단어로 요약
            5. Sources 섹션에 실제 URL 포함하여 작성
            6. 외국인이 이해하기 쉽게 한국 문화적 맥락과 배경 설명 포함
            7. 외국인이 읽는다는 것을 고려하여 반드시 영어로 번역
            
            
            출력 형식:
            # Korean Issue Summary: [이슈 제목]
            
            **Key Visual Keywords:** ["키워드1","키워드2","키워드3"]
            ...
            **Why It Matters:**
            ...
            이 요약본은 밈코인에 관심있는 외국인들이 한국 이슈의 배경을 이해할 수 있도록 도와주는 자료입니다.

            **Sources:**
            [언론사명] - [기사제목] - [URL]
         
            참조한 기사의 언론사명, 기사의 제목, 출처 URL을 영어로 작성합니다.
            취합에 사용된 모든 기사의 출처를 반드시 포함하세요.
            가능하면 2-3개 이상의 서로 다른 언론사 출처를 제공해야 합니다.
            실제 참조한 기사 수만큼 모두 나열하세요.
            ...
            주의사항: [URL Placeholder]가 아닌 실제 URL을 사용하세요.

            """,
            expected_output="영어로 번역된 한국 이슈 종합 요약본",
    ...
    def create_summary_task(self):
        return Task(
            description="""
            이전 단계에서 생성된 모든 정보(기사 요약글, 풍자글 + 토크노믹스 + 로드맵)를 웹사이트 봇이 사용할 수 있는 JSON 형식으로 변환하세요.
            ...
            JSON 구조 요구사항:
            {
                ...
                "korean_issue_summary": {
                    "title": "이슈 제목",
                    "background": "한국의 문화적/사회적 맥락 설명",
                    "what_happened": "현재 상황과 주요 사건들",
                    "key_points": ["핵심 포인트 1", "핵심 포인트 2", "핵심 포인트 3"],
                    "why_it_matters": "이 이슈가 왜 중요하고 주목받는지"
                },
                "sources": [
                    {
                        "outlet": "언론사명",
            ...
            5. "description" 섹션은 요약하지 말고 풍자글의 핵심인 매력적인 부분을 그대로 발췌하세요.
            """,
    ...
    def run_satire_generation(self, keyword: str, why_trending: str):
        ...
        summary_task.context = [translation_task, tokenomics_task]

        # 2. Crew를 생성하고 모든 Task를 순서대로 실행합니다.
        crew = Crew(
            agents=[self.search_agent, self.extractor_agent, self.translator_agent, self.writer_agent, self.tokenomics_agent, self.summary_agent],
            tasks=[search_task, extraction_task, translation_task, satire_task, tokenomics_task, summary_task],
        ...
        try:
            ...
            try:
                partial_results = self._get_partial_results([search_task, extraction_task, translation_task, satire_task, tokenomics_task, summary_task])
    ...
    def _get_partial_results(self, tasks):
        ...
        task_names = ['search', 'extraction', 'translation', 'satire', 'tokenomics', 'summary']
...
def main():
    ...
    # 팀원 에이전트로부터 받은 입력 예시
    keyword = "이춘석"
    why_trending = "이춘석 is a South Korean politician. Recent news suggests he is under investigation for alleged stock trading using privileged information."
